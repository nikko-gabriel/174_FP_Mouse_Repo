...
while True:
    ...
    # Apply foreground mask to color frame
    hsv_frame = cv2.bitwise_and(hsv_frame, hsv_frame, mask=fg_mask)
    ...
    if green_bounding_box is not None:
        mouse_sensitivity = 3  # Increase sensitivity for faster cursor movement
        cursor_x = (green_bounding_box[0] + green_bounding_box[2] // 2) * mouse_sensitivity
        cursor_y = (green_bounding_box[1] + green_bounding_box[3] // 2) * mouse_sensitivity
        mouse.position = (cursor_x, cursor_y)

    # Define the minimum distance threshold for click action
    click_distance_threshold = 50

    if red_bounding_box is not None and blue_bounding_box is not None:
        red_center = (red_bounding_box[0] + red_bounding_box[2] // 2, red_bounding_box[1] + red_bounding_box[3] // 2)
        blue_center = (blue_bounding_box[0] + blue_bounding_box[2] // 2, blue_bounding_box[1] + blue_bounding_box[3] // 2)
        distance = np.linalg.norm(np.array(red_center) - np.array(blue_center))
    ...
    if red_bounding_box is not None and yellow_bounding_box is not None:
        red_center = (red_bounding_box[0] + red_bounding_box[2] // 2, red_bounding_box[1] + red_bounding_box[3] // 2)
        yellow_center = (yellow_bounding_box[0] + yellow_bounding_box[2] // 2, yellow_bounding_box[1] + yellow_bounding_box[3] // 2)
        distance = np.linalg.norm(np.array(red_center) - np.array(yellow_center))
        if distance <= click_distance_threshold:
